...
public class BotController implements PlayerController {
    ...
    public void onActionChance(Seat seat, int tile, List<Action> options, Responder r) {
        for (Action a : options) {
            if (a.type == Action.Type.HU) { r.act(a); return; }
        }
        for (Action a : options) {
            if (a.type == Action.Type.GANG) { r.act(a); return; }
        }
        for (Action a : options) {
            if (a.type == Action.Type.PENG) { r.act(a); return; }
        }
        for (Action a : options) {
            if (a.type == Action.Type.CHI) { r.act(a); return; }
        }
        r.pass();
    }
    ...
    public void onDrawChance(Seat seat, int drawnTile, List<Action> options, Responder r) {
        ...
        for (Action a : options) {
            if (a.type == Action.Type.AN_GANG || a.type == Action.Type.BU_GANG) { r.act(a); return; }
        }
        r.pass();
    }
