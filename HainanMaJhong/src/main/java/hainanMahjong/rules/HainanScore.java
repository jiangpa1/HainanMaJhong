...
public final class HainanScore {
    ...
    public static Settlement settleRound(RoundResult r, Seat dealer, int bottom, HainanConfig cfg,
        ...
        /*
         * 代付原因要记下来，而且必须区分是哪一种"包"——
         * 界面上要直接告诉玩家"这局为什么是我一个人赔三家"。
         * 判定的先后顺序与原来一致（尾墙 > 三道 > 抢杠；自摸走四道）。
         */
        String baoReason = null;
        if (r.from != null) {
        ...
        if (responsible == null) {
            ...
            // 具体是哪种"包"由上面的判定给出；带上"代付三家"让人一眼看懂金额
            notes.get(responsible).add(baoReason + "·代付三家");
        }
    ...
    /**
     * 两个座位之间单向吃碰杠的<b>最大</b>次数。
     *
     * <p>"三道/四道"就是拿这个数和 3/4 比：某一方从另一方吃碰杠满 3 次（点炮包）
     * 或满 4 次（自摸包），这一对之间就成立包牌关系。</p>
     *
     * <p>结算判定和局内实时播报都用这一个方法 —— 两边各算一套的话，
     * 迟早会出现"播报说包了、结算却不包"这种自相矛盾的情况。</p>
     */
    public static int relationCount(Seat a, Seat b, Map<Seat, List<Meld>> melds) {
        if (a == null || b == null || melds == null) {
            return 0;
        }
        int ab = 0, ba = 0;
    ...
    private static boolean relationAtLeast(Seat winner, Seat donor, Map<Seat, List<Meld>> melds, int n) {
        return relationCount(winner, donor, melds) >= n;
    }

    /** 抢杠胡且该胡没有任何其它有番（杠者应独担三家）。 */
    private static boolean qiangNoOtherFan(RoundResult r) {
        HainanFan.Gate g = HainanFan.gate(envOf(r));
