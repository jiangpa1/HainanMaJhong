package hainanMahjong.engine.bot;
...
public class BotController implements PlayerController {
    ...
    private int pickDiscard(List<Integer> hand) {
        ...
        for (int t : hand) {
            cnt.compute(t, (k, c) -> c == null ? 1 : c + 1);
        }

        List<Integer> honorSingle = new ArrayList<>();
        List<Integer> suitIsolated = new ArrayList<>();
        List<Integer> anySingle = new ArrayList<>();
