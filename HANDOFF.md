...
### 规则实现口径（已在代码/自检落地）
- **庄/令/底分**：首局随机庄；庄胡牌或流局→连庄且底分+1；非庄胡→下庄（新庄=原庄下家、底分重置）。“令”东起步，跟首局庄**第 2/3/4 次重新上庄**换南/西/北（连庄不推进令）。一轮=**打满四风**（首局庄完成第 4 庄）结束。
- **有番门禁**：结构合法且满足任一有番条件才能胡（门清/中发白刻/令风刻/座位风刻/四顺子带非字将/位置花/特殊牌型/自摸杠开/抢杠/杠/2·5·8将）。
- **结算**：三家都付、每笔“含庄→按当前庄底分、纯闲→×1”；点炮者另赔固定 1 分；基础分胡1/自摸2/杠开3；番型倍数命中相加，平胡仅无其它番型时兜底=1，龙七对只按5覆盖七对。流局不计胡/杠/花分（跟牌除外）。
- **杠钱/花分**：明1补1暗2，三种杠都三家各付且含庄翻倍；花分(真花1/假0/真对0/假对0可设)同理三家各付且含庄翻倍；花分各自独立累加、真/假花不互斥。
...
- **天胡/天听/地听/报听**：天胡=当局从未出牌摸牌即胡(×5)；听牌判定=结构能胡即可(不计番)；天听=自己只出1张后可报听(×4)、地听=庄首张~第4张前无人吃碰杠可报听(×2)，同满足取最高；报听后锁手：只能胡/自摸/暗杠，摸的牌自动打出；机器人自动报听、真人点“报听”按钮。
- **风/花/座位**：每局庄=东、下家=南、对家=西、上家=北；位置花 庄=梅春、下=兰夏、对=竹秋、上=菊冬；对花拆真对花/假对花。
...
`9ce64f9` 凭据环境变量化+.gitignore+清理 · `295873c` 牌墙剩余/报听说明/战绩口径 · `f203ff0` 四人全退解散房间 · 之前：前端(ec4909a)/服务持久化(fe2144c)/引擎规则(1dbbb19) 等。
